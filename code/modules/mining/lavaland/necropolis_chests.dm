...
/obj/item/rod_of_asclepius/attack_self(mob/user)
	...
	if(itemUser.has_status_effect(STATUS_EFFECT_HIPPOCRATIC_OATH))
		...
		return
	var/failText = span_warning("The snake seems unsatisfied with my incomplete oath and returns to its previous place on the rod, returning to its dormant, wooden state. You must stand still while completing my oath!")
	to_chat(itemUser, span_notice("The wooden snake that was carved into the rod seems to suddenly come alive and begins to slither down my arm! The compulsion to help others grows abnormally strong..."))
	...
	if(do_after(itemUser, 30, target = itemUser))
		...
		to_chat(itemUser, failText)
		return
	to_chat(itemUser, span_notice("The snake, satisfied with my oath, attaches itself and the rod to my forearm with an inseparable grip. Your thoughts seem to only revolve around the core idea of helping others, and harm is nothing more than a distant, wicked memory..."))
...
/obj/item/dragons_blood/attack_self(mob/living/carbon/human/user)
	...
	switch(random)
		if(1)
			...
			H.dna.features = MANDATORY_FEATURE_LIST
			H.eye_color = "fee5a3"
		...
		if(3)
			to_chat(user, span_danger("Power courses through you! You can now shift my form at will."))
			if(user.mind)
...
/obj/item/hierophant_club/ui_action_click(mob/user, action)
	...
	if(!user.is_holding(src)) //you need to hold the staff to teleport
		to_chat(user, span_warning("I need to hold the club in my hands to [beacon ? "teleport with it":"detach the beacon"]!"))
		return
	...
	if(get_dist(user, beacon) <= 2) //beacon too close abort
		to_chat(user, span_warning("I are too close to the beacon to teleport to it!"))
		return
